...
import { PerformanceMetrics, ResourceCount } from '../../types/analysis';
...
export class PerformanceAnalyzer {
   ...
   * 统计资源数量
   */
  private countResources(): ResourceCount {
   ...
   * 验证性能指标
   */
  public validatePerformance(metrics: PerformanceMetrics): {
    isValid: boolean;
    issues: string[];
   ...
   * 获取性能优化建议
   */
  public getOptimizationSuggestions(metrics: PerformanceMetrics): {
    loadingOptimizations: string[];
    resourceOptimizations: string[];
    renderingOptimizations: string[];
    cacheOptimizations: string[];
  } {
    const loadingOptimizations: string[] = [];
   ...
   * 获取性能评分
   */
  public calculatePerformanceScore(metrics: PerformanceMetrics): {
    overall: number;
    loading: number;
    ...
    if (metrics.loadTime > 3000) loadingScore -= 30;
    else if (metrics.loadTime > 2000) loadingScore -= 15;
    
    if (metrics.firstContentfulPaint && metrics.firstContentfulPaint > 1800) loadingScore -= 20;
    else if (metrics.firstContentfulPaint && metrics.firstContentfulPaint > 1200) loadingScore -= 10;

    // 交互性评分
    let interactivityScore = 100;
    if (metrics.firstInputDelay && metrics.firstInputDelay > 100) interactivityScore -= 25;
    else if (metrics.firstInputDelay && metrics.firstInputDelay > 50) interactivityScore -= 10;

    if (metrics.resourceCount.scripts > 20) interactivityScore -= 15;

    // 视觉稳定性评分
    let visualStabilityScore = 100;
    if (metrics.cumulativeLayoutShift && metrics.cumulativeLayoutShift > 0.1) visualStabilityScore -= 30;
    else if (metrics.cumulativeLayoutShift && metrics.cumulativeLayoutShift > 0.05) visualStabilityScore -= 15;
