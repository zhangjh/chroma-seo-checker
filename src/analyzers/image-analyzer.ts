...
import { ImageAnalysis, ImageInfo } from '../../types/analysis';
...
export class ImageAnalyzer {
   ...
   * 分析页面图片
   */
  public async analyzeImages(): Promise<ImageAnalysis> {
    ...
    const largeImages = imageInfos.filter(img => 
      img.fileSize && img.fileSize > 500000 // 大于500KB的图片
    );
   ...
   * 分析单个图片
   */
  private async analyzeImage(img: HTMLImageElement): Promise<ImageInfo> {
   ...
   * 估算图片文件大小
   */
  private async estimateFileSize(img: HTMLImageElement): Promise<number | undefined> {
    try {
      // 尝试通过canvas估算文件大小
   ...
   * 检查图片是否可能损坏
   */
  private isPotentiallyBroken(src: string): boolean {
    if (!src) return true;
    
    // 检查明显的错误格式
    if (src === '#' || src === 'javascript:void(0)') return true;
    
    // 检查是否为data URL但格式错误
    if (src.startsWith('data:') && !src.includes('base64')) return true;
    
    // 检查是否为相对路径但格式可疑
    if (src.startsWith('./') && src.length < 5) return true;
   ...
   * 验证图片SEO最佳实践
   */
  public validateImageSEO(analysis: ImageAnalysis): {
    isValid: boolean;
    issues: string[];
   ...
   * 获取图片优化建议
   */
  public getOptimizationSuggestions(analysis: ImageAnalysis): {
    compressionSuggestions: string[];
    formatSuggestions: string[];
    altTextSuggestions: string[];
    performanceSuggestions: string[];
  } {
    const compressionSuggestions: string[] = [];
   ...
   * 获取图片统计信息
   */
  public getImageStatistics(analysis: ImageAnalysis): {
    altCoverage: number;
    formatDistribution: Record<string, number>;
    sizeDistribution: {
      small: number;    // < 50KB
      medium: number;   // 50KB - 200KB
      large: number;    // 200KB - 500KB
      extraLarge: number; // > 500KB
    };
    totalSize: number;
  } {
    ...
    analysis.largeImages.forEach(img => {
      if (img.fileSize) {
        if (img.fileSize < 50000) sizeDistribution.small++;
        else if (img.fileSize < 200000) sizeDistribution.medium++;
        else if (img.fileSize < 500000) sizeDistribution.large++;
        else sizeDistribution.extraLarge++;
      }
    });

    const totalSize = analysis.largeImages.reduce((sum, img) => 
