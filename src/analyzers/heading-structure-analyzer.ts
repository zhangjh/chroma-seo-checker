...
import { HeadingStructure, HeadingHierarchy } from '../../types/analysis';
...
export class HeadingStructureAnalyzer {
   ...
   * 分析页面标题结构
   */
  public analyzeHeadingStructure(): HeadingStructure {
   ...
   * 根据标题级别提取标题文本
   */
  private extractHeadingsByLevel(level: string): string[] {
   ...
   * 构建标题层次结构
   */
  private buildHeadingHierarchy(): HeadingHierarchy[] {
   ...
   * 生成元素的CSS选择器
   */
  private generateCSSSelector(element: Element, index: number): string {
   ...
   * 验证标题结构的SEO最佳实践
   */
  public validateHeadingStructure(structure: HeadingStructure): {
    isValid: boolean;
    issues: string[];
   ...
   * 验证标题层次结构的逻辑性
   */
  private validateHierarchy(hierarchy: HeadingHierarchy[]): {
    issues: string[];
    recommendations: string[];
  } {
    const issues: string[] = [];
   ...
   * 验证标题长度
   */
  private validateHeadingLengths(structure: HeadingStructure): {
    issues: string[];
    recommendations: string[];
  } {
    const issues: string[] = [];
   ...
   * 验证标题内容质量
   */
  private validateHeadingContent(structure: HeadingStructure): {
    issues: string[];
    recommendations: string[];
  } {
    const issues: string[] = [];
   ...
   * 获取标题结构统计信息
   */
  public getHeadingStatistics(structure: HeadingStructure): {
